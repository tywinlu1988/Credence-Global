...
- **index_compliance**: When a Delivery Note's `rendered` list (excluding `report-index.html` itself) contains more than 2 report files, `dev/templates/report-index.html` must be present and must link to each report with a relative path.

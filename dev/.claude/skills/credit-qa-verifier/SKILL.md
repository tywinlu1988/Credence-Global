...
- **Chain compliance**: A Path Sheet exists for the `path_id`, and this QA Verdict is produced before delivery — no analysis ships without it. All artifact files (Path Sheet / Analysis Artifact / Delivery Note) must parse as valid YAML — an unparseable artifact is a `fail`. Rule source: `dev/engine/pipeline-contract.md`.

...
> **Cross-document consumption note:** When the Concentration Framework references the Contagion Matrix, it only consumes each cell's **`intensity`** and **`direction`** fields, used to identify contagion strength and determine uni/bidirectional relationships. The `type`, `confidence`, and `historical_cases` fields do not enter the quantitative calculation in the current v0.2.0.

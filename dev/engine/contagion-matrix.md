...
根据《传染理论基础》(v0.7.0-release)定义的6种分析范式，将13个行业映射至对应范式。同一范式内的行业因共享相同的风险驱动因子、融资特征和市场叙事逻辑，**先天性具有更高的传染耦合度**。

### 1.2 范式映射表

| 序号 | 行业 | 范式归属 | 范式核心特征 | 金融属性 | 国企占比 |
|------|------|----------|-------------|---------|---------|
| 1 | 光伏/储能 | **范式A** 政策驱动型 | 政策周期决定行业总需求·设备/材料进口依赖·产能周期波动大 | 中等 | 中等(央企/地方国企参股) |
| 2 | 半导体/集成电路 | **范式A** 政策驱动型 + **范式B** 技术壁垒型 | 双重属性：政策资金驱动+技术壁垒·国产替代主线·资本开支巨大 | 高 | 中等(大基金/国资背景) |
| 3 | 高端装备/工业母机 | **范式B** 技术壁垒型 | 技术密集型·客户定制化·进口替代逻辑·周期性与政策关联 | 中等 | 中等(部分央企) |
| 4 | 生物医药/创新药 | **范式B** 技术壁垒型 | 研发驱动·管线价值决定信用·融资依赖IPO/私募·高不确定性 | 中高 | 低(民企为主) |
| 5 | 医疗器械 | **范式B** 技术壁垒型 | 技术壁垒+集采政策影响·设备+耗材双轮驱动·国产替代 | 中等 | 中等(部分国企) |
| 6 | 新能源汽车 | **范式C** 存量博弈型 | 产能过剩·价格战·政策补贴退坡·同质化竞争·尾部出清 | 高 | 低(民企为主) |
| 7 | 数据中心/算力基建 | **范式D** 资产租约型 | 重资产·大客户租约驱动·REITs/ABS融资·NOI决定偿债 | 极高 | 中等(国企+民企) |
| 8 | 城投债(LGV) | **特殊：政府信用绑定型** | 地方政府信用背书·区域财政决定信用·基础设施投资载体 | 极高(金融工具属性) | 100%(地方政府平台) |
| 9 | 食品饮料 | **范式E** 品牌+渠道型 | 品牌壁垒·渠道为王·现金流稳定·消费者信任敏感 | 低 | 极低(民企为主) |
| 10 | 纺织服装 | **范式E** 品牌+渠道型 | 品牌+代工双模式·出口导向·季节性·消费降级敏感 | 低 | 极低(民企为主) |
| 11 | 交通运输 | **范式F** 网络+流量型 | 基础设施属性·重资产·高杠杆·现金流稳定·国企占比高 | 高 | 高(铁路/公路/港口/航空多为国企) |
| 12 | 商贸零售 | **范式F** 网络+流量型 | 渠道价值·现金流好·线上线下融合·消费波动敏感 | 中等 | 低(民企+混合所有制) |
| 13 | 传媒/互联网 | **范式F** 网络+流量型 | 流量驱动·平台效应·广告/付费收入·监管敏感·马太效应 | 中高 | 低(民企为主) |

### 1.3 范式内聚类

| 范式 | 纳入行业 | 范式内传染特征 |
|------|---------|---------------|
| **范式A** (政策驱动型) | 光伏/储能、半导体/集成电路 | 政策转向同步冲击·产能周期共振·融资环境同频 |
| **范式B** (技术壁垒型) | 半导体/集成电路、高端装备、生物医药、医疗器械 | 供应链紧密·技术路线迭代风险·IPO融资渠道共振 |
| **范式C** (存量博弈型) | 新能源汽车 | (该范式仅新能源车一个行业，无范式内传染) |
| **范式D** (资产租约型) | 数据中心/算力基建 | (该范式仅数据中心一个行业，无范式内传染) |
| **范式E** (品牌+渠道型) | 食品饮料、纺织服装 | 消费信心共振·品牌危机示范效应·代工厂供应链传染 |
| **范式F** (网络+流量型) | 交通运输、商贸零售、传媒/互联网 | 网络效应交叉·平台生态捆绑·物流/信息流/资金流三流合一 |
| **特殊** (政府信用绑定型) | 城投债(LGV) | 通过区域财政和基础设施投资与多行业广泛连接 |
...
- **范式A内的弱连接**：光伏→高端装备(3,已完成)→其余范式内链均≤2
- **范式B内的弱连接**：高端装备→生物医药(2)、高端装备→医疗器械(2)、生物医药→高端装备(2)
- **范式C存量博弈型**：新能源车对其他行业的传导主要集中在范式A(光伏/半导体)的共享上游，对其他消费类行业传导极弱
- **范式D资产租约型**：数据中心作为"基础设施"行业，对商贸零售(2)和交通运输(1)有弱下游传导
- **范式E品牌+渠道型**：食品饮料↔纺织服装(2)主要通过信心崩塌、食品饮料↔商贸零售(2)、纺织服装↔商贸零售(2)、纺织服装→传媒互联网(2)
- **范式F网络+流量型**：交通运输↔商贸零售(2)、交通运输↔传媒互联网(2)，均为同范式内弱连接
...
| 范式 | 行业组合 | 强度 | 逻辑依据 |
|------|---------|------|---------|
| 范式A(政策驱动型) | 光伏↔半导体 | 4 | 共享政策资金支持·同受产业政策周期驱动·设备/材料进口依赖一致·产能周期同步·同为"新质生产力"标签 |
| 范式B(技术壁垒型) | 生物医药↔医疗器械 | 4 | 同一监管体系(集采/医保)·共享医院终端·研发驱动同质·均为技术替代风险暴露 |
| 范式B(技术壁垒型) | 高端装备↔半导体 | 4 | 装备是半导体的上游·进口替代双主线·同为制造强国战略核心 |
| 范式E(品牌+渠道型) | 食品饮料↔纺织服装 | 2 | 共享"消费信心"属性·品牌危机示范效应·但实质性信用链关联弱 |

**例外说明：** 范式F(网络+流量型)内交通运输、商贸零售、传媒互联网三行业之间传导强度最高仅达3(商贸零售↔传媒互联网)，低于范式A和B内部的4分。原因：交通运输偏基础设施属性，与商贸零售/互联网的流量属性虽有连接但耦合度不足以达到强传染水平。
...
食品饮料(范式E)和纺织服装(范式E)之间的传导强度仅为2，主要通过信心崩塌(品牌危机示范效应)而非实质性信用链：
...
| 类型 | 示例 | 解释 |
|------|------|------|
| 上下游不对称 | 半导体→医疗器械(3) vs 医疗器械→半导体(1) | 半导体是上游，对下游影响大；下游违约对上游影响有限 |
...
**触发条件：** VIX>30 或 信用利差走阔>50bp 或 一级市场取消发行占比>20%
...
|---------|--------------|------------|-------------|---------|
| 市场恐慌情绪 | 15-20% | 3月/8月/11月 | VIX指数·信用利差·取消发行率 | VIX>25·利差跳升>30bp |
...
| 金字塔层级 | 集成方式 | 具体操作 |
|-----------|---------|---------|
| 集中度压力测试 | 使用矩阵做**组合传染模拟**(详见7.2) | 输入假想违约→矩阵驱动传播→输出组合影响 |
| 行业集中度限制 | 在集中度管理中增加**传染关联集中度** | 不仅是单一行业集中度，还要考虑集群A+B+C的**总暴露敞口** |
| 限额管理系统 | 在限额中增加**传染路径限额** | 对高传染链路(A→B)设定额外限额扣减 |
...
| v0.7.0-release | 2026-07-10 | 系统智能层整合：引擎版本统一为v0.7.0-release，与contagion-theory形成完整传染图谱体系 | 引擎团队 |

---

*本文档是《传染理论基础》(v0.7.0-release)的操作化延伸，两者需配合使用。*

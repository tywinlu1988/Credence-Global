...
**版本**: v0.6.8-alpha | **日期**: 2026-07-10 | **状态**: 初始发布
...
| 版本 | 计划内容 |
|------|---------|
| v0.6.8-alpha (当前) | 初始版本：基本聚合算法+温度计+3个历史回测+2026Q2即时计算 |
| v0.6.9-beta | 增加SRI时间序列追踪（绘制SRI历史曲线，识别趋势和拐点） |
| v0.7.0-beta | 引入SRI与传染矩阵升级因子的实时联动（当升级因子触发时自动调整SRI读数） |
...
*本文档应与《双轨分析方法论》(v0.4.0)、《传染矩阵》(v0.6.3-alpha)、《五维集中度分析框架》(v0.6.5-alpha)、《展望监控框架》配合使用。系统性预警框架是引擎M4组合风控层的顶层仪表盘，为分散的行业信号提供统一的系统性风险读数。*

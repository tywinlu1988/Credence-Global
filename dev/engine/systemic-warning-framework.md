...
其中：
  ...
  归一化：最终的行业权重在各行业间归一化，确保 Σ(行业权重) = 13
```
...
```
半导体行业权重 = 1.5% × 1.295 = 1.94%

归一化处理：
  各行业原始权重 = 信用债存量占比 × 传染力系数
  归一化因子 = 13 / Σ(原始权重)
  最终权重 = 原始权重 × 归一化因子
```
...
- 归一化权重 = 原始权重 / 68.41% × 61.33%
- 归一化因子 = 13 / Σ(归一化权重)（确保最终和为13）
...
| v0.7.0-release (当前) | 系统智能层整合：与传染矩阵/集中度框架形成完整M4组合风控体系，引擎版本统一 |
...
| v0.7.0-release | 2026-07-10 | 系统智能层整合：引擎版本统一为v0.7.0-release，与传染矩阵/集中度框架形成完整M4组合风控体系 | 引擎团队 |

---

*本文档应与《双轨分析方法论》(v0.7.0-release)、《传染矩阵》(v0.7.0-release)、《五维集中度分析框架》(v0.7.0-release)、《展望监控框架》配合使用。系统性预警框架是引擎M4组合风控层的顶层仪表盘，为分散的行业信号提供统一的系统性风险读数。*

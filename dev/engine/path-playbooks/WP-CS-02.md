...
1. `dev/engine/lgd-recovery-framework.md` — LGD five-tier classification, collateral valuation, recovery path
2. `dev/engine/external-support-framework.md` — government/group/strategic support assessment
...
Analysis Artifact per `dev/engine/pipeline-contract.md` §2.2 (add-on refinement, layered on WP-CS-01 base).
Path outputs (registry): LGD tier + recovery rate, external support adjustment recommendation.

## 6. Templates

- `dev/templates/template-type8.html` — LGD Assessment
- `dev/templates/template-type9.html` — External Support

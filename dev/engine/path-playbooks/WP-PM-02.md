...
Analysis Artifact per `dev/engine/pipeline-contract.md` §2.2 (dual-issuer, with per-issuer findings arrays).
Path outputs (registry): comparison score, differentiation conclusion.

## 6. Templates

- `dev/templates/template-type2.html` — Comparative Analysis

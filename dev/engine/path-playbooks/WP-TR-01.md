...
1. `dev/engine/trader-framework.md` §§1-5 — Execution Dimensions, Track B mapping, Thermometer Overlay, Execution Decision Matrix (this path's own methodology)
2. `dev/engine/output-layered-framework.md` §§1-3 — L0 Signal Card specification (§3), three-tier overview; §6 — information priority sorting rules
3. `dev/engine/systemic-warning-framework.md` §§1-4 — SRI four-tier thermometer (§3), signal aggregation, escalation trigger definitions
...
3. **Score Execution Dimensions** — Score the four dimensions (L0 Signal Card 35% / Real-Time Spreads 25% / Liquidity Conditions 25% / Market Context 15%) per `dev/engine/trader-framework.md` §2; apply the Track B input mapping (§3) and the liquidity data-gap rule (§2.4).
4. **Select Execution Decision Matrix row** — Follow the selection procedure in `dev/engine/trader-framework.md` §5.2: composite → signal direction → spread band → liquidity → matrix row.
5. **Read SRI thermometer + apply overlay** — Read the current systemic risk thermometer tier from `systemic-warning-framework.md` §3 (🟢 Normal <0.5 / 🟡 Watch 0.5-1.0 / 🟠 Alert 1.0-1.8 / 🔴 Danger ≥1.8; most recent WP-RO-03 reading). Apply the overlay per `dev/engine/trader-framework.md` §4 — Alert/Danger overrides any Buy row to "suspended".
...
- Execution Dimensions: L0 Signal Card 35% / Real-Time Spreads 25% / Liquidity Conditions 25% / Market Context 15% per `dev/engine/trader-framework.md` §2 only.
- Spread bands: Cheap > FV+10bp / Fair ±10bp / Rich < FV−10bp per `dev/engine/trader-framework.md` §2.3 only.
- Execution posture: one line, from the Execution Decision Matrix (`dev/engine/trader-framework.md` §5) after the §4 thermometer overlay.
...
Analysis Artifact per `dev/engine/pipeline-contract.md` §2.2 (L0 — lightest artifact, signals-only).
...
- `"L0-spec: dev/engine/output-layered-framework.md §3"` — L0 Signal Card (specification-defined, no standalone .html file)
...
- Inventing execution thresholds, weights, or matrix rows not defined in `dev/engine/trader-framework.md`.
- Fabricating liquidity, spread, or market-depth data — missing data triggers the data-gap rule (`dev/engine/trader-framework.md` §2.4), never assumed values; Mode B guardrail applies (`mosaic-engine.md` §6).
- Issuing trading advice instead of an analytical execution posture (`dev/engine/trader-framework.md` §9 posture).

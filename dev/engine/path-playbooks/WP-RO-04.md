...
1. `dev/engine/concentration-framework.md` — stress scenario definitions (§9), five-dimension thresholds as stress inputs
2. `dev/engine/financial-deep-dive.md` — scenario sensitivity matrix (§E), stress effects on financial statements
...
Analysis Artifact per `dev/engine/pipeline-contract.md` §2.2 (multi-scenario, with per-scenario findings arrays).
Path outputs (registry): stress scenario loss, threshold jump results.

## 6. Templates

- `dev/templates/template-type11.html` — Stress Test

...
**Corresponding Engine Version**: v0.8.4-release
...
**Snapshot Boundary**: Since **v0.8.0-release**, `version/<version>/` = **installable agent package output by `scripts/build_dist.py`** (= contents of `dist/credence/`): skills placed in `.claude/skills/`, `engine/` flattened, `templates/`, `src/`, `adapters/`, plus generated `AGENTS.md`/`CLAUDE.md`/`GEMINI.md`/`INSTALL.md`/`README.md`/`.claude-plugin/plugin.json`. This package has removed dead links and root directory assumptions (removed `settings.local.json` absolute paths, `engine/audits/`, `design/`, `product/`, `data/`, `validation/`, and pattern-removed 16 audits/validation trace pointers). **Exception**: `v0.8.0-alpha` used the old mirrored three-root layout (`dev/` + `AGENTS.md` + `src/`), an integration pre-release that was not made installable. `validation/` artifacts never enter snapshots; `version/` (historical snapshots), `scripts/`, `tests/`, `docs/`, and any `.git`/`__pycache__`/`*.pyc` also do not enter snapshots.
...
- [ ] Skill package has been placed in `version/<version>/.claude/skills/` via build (since v0.8.0-release at package root `.claude/skills/`, generated by `scripts/build_dist.py`).
...
3. Copy the contents of `dist/credence/` to `version/<version>/` (**since v0.8.0-release**; `dist/` itself is a gitignored build artifact, the committed snapshot is in `version/`).
...
> **Historical Snapshot Differences**: Snapshots ≤ v0.7.1-release were flat copies of `dev/` single-root (`version/<v>/` top-level equals `dev/` contents); `v0.8.0-alpha` was a **mirrored three-root** layout (`dev/` + `src/` + `AGENTS.md`, not installable); since **v0.8.0-release**, the layout has changed to the installable package format above.

...
**Corresponding Engine Version**: v0.3.1
...
- [ ] `python scripts/build_dist.py --zip` build + built-in validation passes (zero absolute paths, zero dev/ tokens, all links resolvable, 4 skills with strict frontmatter, 27 shipped CORE_DOCS complete (appendix/ and reference/ stay repo-only)) and produces `version/<v>-release.zip` + `.sha256`.

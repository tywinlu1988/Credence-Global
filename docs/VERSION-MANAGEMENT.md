...
**Corresponding Engine Version**: v0.1.1
...
- [ ] `python scripts/build_dist.py --zip` build + built-in validation passes (zero absolute paths, zero dev/ tokens, all links resolvable, 4 skills with strict frontmatter, 30 CORE_DOCS complete) and produces `version/<v>-release.zip` + `.sha256`.

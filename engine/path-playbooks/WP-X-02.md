...
1. `${CLAUDE_PLUGIN_ROOT}/engine/multi-stakeholder.md` §§1-4 — role definitions (§1), role deep-dives (§2), cross-role matrix (§3), five-step parallel process (§4)
2. `${CLAUDE_PLUGIN_ROOT}/engine/industry-framework.md` §§1-5 — per-role underlying analysis basis
...
- `${CLAUDE_PLUGIN_ROOT}/templates/template-type4.html` — Multi-Role Matrix
...
- `Cross-Role Comparison (${CLAUDE_PLUGIN_ROOT}/engine/multi-stakeholder.md §3)`

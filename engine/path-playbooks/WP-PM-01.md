...
1. `${CLAUDE_PLUGIN_ROOT}/engine/multi-stakeholder.md` §2.2b Single-Instrument Dashboard (this path's definition) and §2.2 Portfolio Construction Assessment (distinct lens, do not conflate)
2. `${CLAUDE_PLUGIN_ROOT}/engine/dual-track-methodology.md` §§1-4 — Track A/B foundations for relative value
3. `${CLAUDE_PLUGIN_ROOT}/engine/mosaic-engine.md` §§1-6 — completeness and density rules
...
Analysis Artifact per `${CLAUDE_PLUGIN_ROOT}/engine/pipeline-contract.md` §2.2 (findings: four-dimension scores + investment recommendation; completeness; mode_b_gaps).
Path outputs (registry): four-dimension score + investment recommendation.

## 6. Templates

- `${CLAUDE_PLUGIN_ROOT}/templates/template-type5.html` — PM Dashboard

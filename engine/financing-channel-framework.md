...
- **Channel Comparison (${CLAUDE_PLUGIN_ROOT}/engine/financing-channel-framework.md §3)** — six-factor weighted comparison with all-in after-tax cost calculation, tie-breaking rule, and visibility penalty
- **Timing Assessment (${CLAUDE_PLUGIN_ROOT}/engine/financing-channel-framework.md §4)** — five-factor timing composite with refinancing urgency and issuance-window linkage

...
**Version**: v0.3.0 | **Date**: 2026-07-18

This registry is the design baseline for the v0.0.1 skill architecture refactoring (requirement understanding -> path routing -> engine invocation -> report delivery, all fully controllable). It makes all 16 work paths explicit and machine-readable, serving as the single source of truth for the Intake Router (v0.3.0) routing and execution-layer skill decomposition (v0.3.0).
...
> **Reading guide**: This is a routing table — read as needed. It does not contain executable scoring
...
depth: L2
engine_sequence:
  - ${CLAUDE_PLUGIN_ROOT}/engine/industry-framework.md
  - ${CLAUDE_PLUGIN_ROOT}/engine/mosaic-engine.md
  - ${CLAUDE_PLUGIN_ROOT}/engine/dual-track-methodology.md
...
quality_gates:
  - "Signal Density (${CLAUDE_PLUGIN_ROOT}/engine/mosaic-engine.md §4.3)"
...
depth: special
engine_sequence:
  - ${CLAUDE_PLUGIN_ROOT}/engine/lgd-recovery-framework.md
  - ${CLAUDE_PLUGIN_ROOT}/engine/external-support-framework.md
paradigm_selection: n/a (Special add-on module layered on main rating; no paradigm re-selection)
templates:
  - ${CLAUDE_PLUGIN_ROOT}/templates/template-type8.html
...
engine_sequence:
  - ${CLAUDE_PLUGIN_ROOT}/engine/dual-track-methodology.md
  - ${CLAUDE_PLUGIN_ROOT}/engine/validation-methodology.md
...
Underwriter perspective: assesses the feasibility of underwriting a bond issuance -- issuance window judgment, investor matching, and comparable pricing. Engine document (advisor-origination-framework.md) and Type 16 template are delivered. See [Appendix](#6-appendix-development-backlog).
...
depth: L0
engine_sequence:
  - ${CLAUDE_PLUGIN_ROOT}/engine/trader-framework.md
  - ${CLAUDE_PLUGIN_ROOT}/engine/output-layered-framework.md
  - ${CLAUDE_PLUGIN_ROOT}/engine/systemic-warning-framework.md
paradigm_selection: n/a (L0 signal layer is cross-paradigm; directly reads single-issuer rating + system thermometer)
templates:
  - "L0-spec: ${CLAUDE_PLUGIN_ROOT}/engine/output-layered-framework.md §3"
...
depth: special
engine_sequence:
  - ${CLAUDE_PLUGIN_ROOT}/engine/concentration-framework.md
paradigm_selection: n/a (Portfolio dimension cross-paradigm aggregation, mapped by holdings industry distribution)
templates:
  - ${CLAUDE_PLUGIN_ROOT}/templates/template-type14.html
outputs: [five-dimension concentration score, concentration adjustment recommendations]
quality_gates:
  - "Five-Dimensional Concentration (${CLAUDE_PLUGIN_ROOT}/engine/concentration-framework.md §1)"
  - "Threshold System (${CLAUDE_PLUGIN_ROOT}/engine/concentration-framework.md §2.2)"
```
...
depth: special
engine_sequence:
  - ${CLAUDE_PLUGIN_ROOT}/engine/contagion-matrix.md
  - ${CLAUDE_PLUGIN_ROOT}/engine/contagion-theory.md
paradigm_selection: n/a (Contagion matrix is cross-paradigm; transmitted by industry pairs)
templates:
  - ${CLAUDE_PLUGIN_ROOT}/templates/template-type13.html
outputs: [contagion path map, contagion adjustment recommendations]
quality_gates:
  - "Contagion Matrix (${CLAUDE_PLUGIN_ROOT}/engine/contagion-matrix.md §2)"
  - "Escalation Factor (${CLAUDE_PLUGIN_ROOT}/engine/contagion-matrix.md §6.1)"
  - "Transmission Path (${CLAUDE_PLUGIN_ROOT}/engine/contagion-theory.md §3)"
```
...
templates:
  - ${CLAUDE_PLUGIN_ROOT}/templates/template-type15.html
outputs: [SRI reading, thermometer tier]
quality_gates:
  - "Signal Aggregation (${CLAUDE_PLUGIN_ROOT}/engine/systemic-warning-framework.md §2)"
...
engine_sequence:
  - ${CLAUDE_PLUGIN_ROOT}/engine/concentration-framework.md
  - ${CLAUDE_PLUGIN_ROOT}/engine/financial-deep-dive.md
paradigm_selection: n/a (Stress scenarios are cross-paradigm)
templates:
  - ${CLAUDE_PLUGIN_ROOT}/templates/template-type11.html
...
Enterprise (issuer) perspective reverse application: compares financing channels (bond/loan/non-standard), judges financing timing and cost. Engine document (financing-channel-framework.md) and Type 17 template are delivered. See [Appendix](#6-appendix-development-backlog).
...
templates:
  - ${CLAUDE_PLUGIN_ROOT}/templates/template-type3.html
outputs: [validation conclusion, framework improvement recommendations]
quality_gates:
  - "Black Swan Back-Testing (${CLAUDE_PLUGIN_ROOT}/engine/validation-methodology.md §1)"
  - "Dual-Timepoint (${CLAUDE_PLUGIN_ROOT}/engine/validation-methodology.md §3)"
```
...
templates:
  - ${CLAUDE_PLUGIN_ROOT}/templates/template-type4.html
outputs: [multi-role score matrix, consensus/divergence report]
quality_gates:
  - "Multi-Role Parallel (${CLAUDE_PLUGIN_ROOT}/engine/multi-stakeholder.md §4)"
  - "Cross-Role Comparison (${CLAUDE_PLUGIN_ROOT}/engine/multi-stakeholder.md §3.2)"
```
...
templates:
  - ${CLAUDE_PLUGIN_ROOT}/templates/template-type7.html
outputs: [industry pyramid, D1-D10 scores]
quality_gates:
  - "Ten-Dimension (${CLAUDE_PLUGIN_ROOT}/engine/industry-framework.md §2)"
  - "Pyramid (${CLAUDE_PLUGIN_ROOT}/engine/industry-framework.md §4)"
  - "Veto (${CLAUDE_PLUGIN_ROOT}/engine/industry-framework.md §5)"
```
...
depth: special
engine_sequence:
  - ${CLAUDE_PLUGIN_ROOT}/engine/esg-framework.md
  - ${CLAUDE_PLUGIN_ROOT}/engine/governance-fraud-risk.md
paradigm_selection: n/a (ESG/governance is a cross-paradigm overlay layer)
templates:
  - ${CLAUDE_PLUGIN_ROOT}/templates/template-type10.html
...
depth: special
engine_sequence:
  - ${CLAUDE_PLUGIN_ROOT}/engine/outlook-monitoring-framework.md
paradigm_selection: n/a (Outlook and monitoring are cross-paradigm mechanisms)
templates:
  - ${CLAUDE_PLUGIN_ROOT}/templates/template-type18.html
outputs: [rating outlook, watchlist]
quality_gates:
  - "Rating Outlook (${CLAUDE_PLUGIN_ROOT}/engine/outlook-monitoring-framework.md §2)"
  - "Watchlist (${CLAUDE_PLUGIN_ROOT}/engine/outlook-monitoring-framework.md §3)"
  - "Rating Migration Matrices (${CLAUDE_PLUGIN_ROOT}/engine/outlook-monitoring-framework.md §5)"
```

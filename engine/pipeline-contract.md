...
findings:                   # one group of findings per engine document / paradigm
  - engine_doc: ""          # ${CLAUDE_PLUGIN_ROOT}/engine/<doc>.md (rule source for this finding group)
...
rendered: []                # Actually produced report files (from ${CLAUDE_PLUGIN_ROOT}/templates/); may include report-index.html when >2 reports
...
chaining_edges:
  - id: edge-l0-to-l1-upgrade
    ...
    source_doc_ref: ${CLAUDE_PLUGIN_ROOT}/engine/output-layered-framework.md §3, §4
  - id: edge-l1-to-l2-upgrade
    from: WP-PM-01
    to: [WP-CS-01]
    trigger: L1 snapshot internal rating differs from external rating by >=2 notches
    source_doc_ref: ${CLAUDE_PLUGIN_ROOT}/engine/output-layered-framework.md §4
  - id: edge-ro-monthly-sri-rerun
    ...
    source_doc_ref: ${CLAUDE_PLUGIN_ROOT}/engine/systemic-warning-framework.md §3
  - id: edge-migration-matrix-rerun
    ...
    to_ref: ${CLAUDE_PLUGIN_ROOT}/engine/work-path-registry.md §5
    ...
    source_doc_ref: ${CLAUDE_PLUGIN_ROOT}/engine/outlook-monitoring-framework.md §4, §5
```

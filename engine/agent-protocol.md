...
2. **No numbers without a citation.** Every threshold, weight, score, tier, or rating
   MUST cite `${CLAUDE_PLUGIN_ROOT}/engine/<doc>.md §section`. If undefined, output `engine_undefined`.
3. **No report without a template.** Deliverable reports MUST be assembled by
   `credit-report-builder` from `${CLAUDE_PLUGIN_ROOT}/templates/`. Never design ad-hoc HTML or layouts.
...
6. **Follow the path's Playbook.** For active paths, `${CLAUDE_PLUGIN_ROOT}/engine/path-playbooks/<path_id>.md`
...
- `${CLAUDE_PLUGIN_ROOT}/engine/pipeline-contract.md` — four-stage I/O contract
- `${CLAUDE_PLUGIN_ROOT}/engine/work-path-registry.md` — 16 work paths, routing table

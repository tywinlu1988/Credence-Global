...
- **Index compliance**: When the Delivery Note's `rendered` list (excluding `report-index.html` itself) contains more than 2 entries, `${CLAUDE_PLUGIN_ROOT}/templates/report-index.html` MUST be present in `rendered`, and every link within it must be a relative path to one of the other rendered files. Rule source: `credit-report-builder` skill §Assembly Protocol step 5.

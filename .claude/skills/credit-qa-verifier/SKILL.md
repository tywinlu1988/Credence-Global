...
- **Delivery Note**: produced by `credit-report-builder` (field shape at `${CLAUDE_PLUGIN_ROOT}/engine/pipeline-contract.md` §2.3).
...
- **QA Verdict** — final stage artifact, field shape at `${CLAUDE_PLUGIN_ROOT}/engine/pipeline-contract.md` §2.4. `verdict` takes one of `pass` | `pass-with-findings` | `fail`.
...
- **Signal density rule `density_rule`**: Dimensions below the density floor must not output numeric scores and must be annotated as `insufficient information to evaluate`; when weighted-average density is insufficient, must not output a final letter rating. Rule source: `${CLAUDE_PLUGIN_ROOT}/engine/mosaic-engine.md` §4.3.
- **One-vote veto ceiling `veto_ceiling`**: Issuers triggering a one-vote veto have their rating ceiling locked at CCC, may not be raised. Rule source: `${CLAUDE_PLUGIN_ROOT}/engine/industry-framework.md` §5.
- **Mode B anti-hallucination `mode_b`**: Unless the user explicitly provides data sources (CSV/API/MCP), no Mode B external data values may appear; all Mode B fields must be treated as data gaps. Rule source: `${CLAUDE_PLUGIN_ROOT}/engine/mosaic-engine.md` §6.
...
- **Template compliance**: Every rendered report file maps to a template in the path's registry `templates` field (or declared marker) — no ad-hoc layouts. Rule source: `${CLAUDE_PLUGIN_ROOT}/engine/work-path-registry.md` + `${CLAUDE_PLUGIN_ROOT}/templates/index.yaml`.
...
- **Dimension compliance**: All analysis dimensions/metrics use engine vocabulary only (industry-framework D1-D10 + paradigm pyramids; concentration-framework five dimensions; contagion-matrix 19 industries; P1-P6 paradigms) — no invented dimensions or industries. Rule source: `${CLAUDE_PLUGIN_ROOT}/engine/industry-framework.md` + `${CLAUDE_PLUGIN_ROOT}/engine/contagion-matrix.md`.
- **Chain compliance**: A Path Sheet exists for the `path_id`, and this QA Verdict is produced before delivery — no analysis ships without it. All artifact files (Path Sheet / Analysis Artifact / Delivery Note) must parse as valid YAML — an unparseable artifact is a `fail`. Rule source: `${CLAUDE_PLUGIN_ROOT}/engine/pipeline-contract.md`.
- **Index compliance**: When the Delivery Note's `rendered` list (excluding `report-index.html` itself) contains more than 1 entry, `report-index.html` MUST be present in `rendered`, and every link within it must be a relative path to one of the other rendered files. Rule source: `credit-report-builder` skill §Assembly Protocol step 6.
- **CSS self-containment**: Every rendered HTML file MUST have `template-base.css` inlined as a `<style>` block — no `<link rel="stylesheet">` tags in delivered reports. Verify by checking that each `.html` file in `rendered` contains `/* inlined from template-base.css */` or the full CSS variable block. Rule source: `credit-report-builder` skill §Assembly Protocol step 4.
- **Relative paths**: All file paths in `rendered` and `source_analysis` must be relative (`./file` or `file`). Absolute paths (starting with a drive letter or root slash) are a protocol violation. Rule source: `credit-report-builder` skill §Assembly Protocol step 7.

## QA Verdict Output

Template (schema single source of truth is `${CLAUDE_PLUGIN_ROOT}/engine/pipeline-contract.md` §2.4):

```yaml
path_id: ""                 # join key (all three artifacts must be consistent)
verdict: ""                 # pass|pass-with-findings|fail
gate_results:               # gate-by-gate review results
  - gate: ""                # "rule name (${CLAUDE_PLUGIN_ROOT}/engine/<doc>.md §section)" (inherited from Path Sheet quality_gates)
    status: ""              # pass|fail
...
gate_results:
  - gate: "Signal Density (${CLAUDE_PLUGIN_ROOT}/engine/mosaic-engine.md §4.3)"
    status: pass
  ...
  - gate: "Veto (${CLAUDE_PLUGIN_ROOT}/engine/industry-framework.md §5)"
    status: pass
    evidence: No one-vote veto triggered; rating ceiling rule verified (industry-framework §5)
  - gate: "Cross-Validation (${CLAUDE_PLUGIN_ROOT}/engine/dual-track-methodology.md §4)"
    status: pass
...
- `${CLAUDE_PLUGIN_ROOT}/engine/pipeline-contract.md` — Four-stage chain I/O contract (artifact schema single source of truth)
- `${CLAUDE_PLUGIN_ROOT}/engine/mosaic-engine.md` — Signal density / completeness / Mode B guardrail (density_rule, mode_b rule source)
- `${CLAUDE_PLUGIN_ROOT}/engine/industry-framework.md` — One-shot veto rating ceiling (veto_ceiling rule source)
- `${CLAUDE_PLUGIN_ROOT}/engine/work-path-registry.md` — Work path registry (quality gate list traceability)

...
The WP-PM-01 dashboard (Type 5 template) evaluates a single bond on four dimensions (weights and metric anchors single-sourced in `${CLAUDE_PLUGIN_ROOT}/engine/multi-stakeholder.md` §2.2b):
...
1. **Received the Work-Path Sheet from router**: Strictly read engine documents in the order listed by the `engine_reading_order` field, and verify each quality gate listed in the `quality_gates` field. The sheet's fields align with the schema of `${CLAUDE_PLUGIN_ROOT}/engine/work-path-registry.md`.
2. **No path sheet (user directly named a specific task)**: Fall back to the core set of the Invocation Protocol — `${CLAUDE_PLUGIN_ROOT}/engine/engine-overview.md` + `${CLAUDE_PLUGIN_ROOT}/engine/dual-track-methodology.md` + the specific topic document requested.

...
**Non-Negotiables (see AGENTS.md)**: no analysis without a Path Sheet · no numbers without a `doc §section` citation (`engine_undefined` otherwise) · no report outside `${CLAUDE_PLUGIN_ROOT}/templates/` · no delivery without a QA Verdict · no invented dimensions or vocabulary · follow the path's Playbook (`${CLAUDE_PLUGIN_ROOT}/engine/path-playbooks/<path_id>.md`).

When this Skill is invoked:

1. **Path-sheet-driven (preferred).** If the user message carries a Path Sheet produced by the `credit-analysis-router` skill, read `${CLAUDE_PLUGIN_ROOT}/engine/path-playbooks/<path_id>.md` first, then the engine documents in the sheet's `engine_reading_order` order, and validate against its `quality_gates`.
2. **Explicit registered path_id.** If the user explicitly names a registered path (e.g., "run WP-RO-01"), treat that as the path selection: load the Playbook, then the core set — `${CLAUDE_PLUGIN_ROOT}/engine/engine-overview.md` + `${CLAUDE_PLUGIN_ROOT}/engine/dual-track-methodology.md` — plus any topic-specific doc the request names.
3. **No Path Sheet and no explicit path_id (strict).** If the request would produce a credit conclusion/rating/score, **STOP — do not analyze.** Route through the `credit-analysis-router` skill first, or ask the Q1–Q4 questions (role / object / depth / data) yourself to pick a path from `${CLAUDE_PLUGIN_ROOT}/engine/work-path-registry.md`. **Exception**: pure knowledge questions (e.g., "what is the SRI formula?", "explain the veto mechanism") may be answered directly from engine documents with citations.
...
# Fixed Income Credit Analysis Engine v0.0.8
...
- Evaluating sovereign-linked or government-supported credit → read `${CLAUDE_PLUGIN_ROOT}/engine/external-support-framework.md`
- Conducting ESG/governance risk scans and fraud detection → read `${CLAUDE_PLUGIN_ROOT}/engine/esg-framework.md` and `${CLAUDE_PLUGIN_ROOT}/engine/governance-fraud-risk.md`
- Performing LGD/recovery rate analysis for default scenarios → read `${CLAUDE_PLUGIN_ROOT}/engine/lgd-recovery-framework.md`
- Assessing external support (government, parent company) impact on creditworthiness → read `${CLAUDE_PLUGIN_ROOT}/engine/external-support-framework.md`
- Evaluating financial bonds → read `${CLAUDE_PLUGIN_ROOT}/engine/financial-bond-framework.md`
- Analyzing holding companies → read `${CLAUDE_PLUGIN_ROOT}/engine/holding-company-framework.md`
...
Full confidence/density model and gap-to-risk mapping: `references/mosaic-engine-architecture.md` (threshold single source: `${CLAUDE_PLUGIN_ROOT}/engine/mosaic-engine.md`).
...
Defined but not implemented. Adapter contract (`query_bond_analytics` / `query_market_data` / `query_industry_benchmark`) and connection priority (CSV > REST API > MCP > DB): `${CLAUDE_PLUGIN_ROOT}/engine/mosaic-engine.md`.

## Veto & Rating Ceiling (mandatory)

- **One-vote veto**: When any analysis layer triggers a one-vote veto condition, the issuer's rating **ceiling is locked at CCC** and may not be raised. Veto conditions for each layer: `${CLAUDE_PLUGIN_ROOT}/engine/industry-framework.md` §5.
- Rating mapping must use the official 12-notch table (`${CLAUDE_PLUGIN_ROOT}/engine/dual-track-methodology.md` §6); do not create custom notches.
- For sovereign-linked entities (P6): fiscal capacity and institutional strength govern, and support willingness determines the final notch (`${CLAUDE_PLUGIN_ROOT}/engine/external-support-framework.md`).
...
Full pyramid weights, Track-B thresholds, and the cross-comparison matrix: `references/industry-scoring.md` and `${CLAUDE_PLUGIN_ROOT}/engine/dual-track-methodology.md`.
...
Full specification: `references/system-intelligence.md` and `${CLAUDE_PLUGIN_ROOT}/engine/systemic-warning-framework.md`.
...
- **Output**: After analysis completes, output the Analysis Artifact (schema at `${CLAUDE_PLUGIN_ROOT}/engine/pipeline-contract.md` §2.2); `path_id` is inherited from the Path Sheet.
...
Details have been moved to `references/` (single source of truth remains `${CLAUDE_PLUGIN_ROOT}/engine/` engine documents):
...
- `.claude/skills/credit-analysis-router/SKILL.md` — Requirement interpretation / routing layer, outputs Path Sheet
- `${CLAUDE_PLUGIN_ROOT}/engine/work-path-registry.md` — Work path registry (path sheet single source of truth)

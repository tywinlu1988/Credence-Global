...
> This table is an assembly mapping view for `credit-report-builder`: given a `path_id`, it lists the templates and primary tier to use. The single source of truth for the template list is `${CLAUDE_PLUGIN_ROOT}/engine/work-path-registry.md` (the `templates` field of each path); the single source of truth for tier semantics is `${CLAUDE_PLUGIN_ROOT}/engine/output-layered-framework.md` §§2/3/5. This table contains no thresholds, tier time budgets, or rating values — all numeric judgments are subject to the referenced engine documents. In case of any inconsistency between this table and those two documents, the latter take precedence.
...
- Three-layer output overview and navigation relationship: `${CLAUDE_PLUGIN_ROOT}/engine/output-layered-framework.md` §2
- L0 Signal Card (layout/elements/thermometer card): `${CLAUDE_PLUGIN_ROOT}/engine/output-layered-framework.md` §3
- L1 Snapshot (four-dimensional radar/key anomalies/rating comparison/ranking): `${CLAUDE_PLUGIN_ROOT}/engine/output-layered-framework.md` §4
- L2 Deep Report (four-panel structure/navigation rules): `${CLAUDE_PLUGIN_ROOT}/engine/output-layered-framework.md` §5
- Completeness indicator presentation: `${CLAUDE_PLUGIN_ROOT}/engine/output-layered-framework.md` §8.4
...
- `L0-spec`: No standalone template file; specification is defined in the referenced engine document (L0 signal card specification see `${CLAUDE_PLUGIN_ROOT}/engine/output-layered-framework.md` §3).

...
**Non-Negotiables (see AGENTS.md)**: no analysis without a Path Sheet · no numbers without a `doc §section` citation · no report outside `${CLAUDE_PLUGIN_ROOT}/templates/` · no delivery without a QA Verdict · no invented dimensions or vocabulary.
...
quality_gates: []           # "rule name (${CLAUDE_PLUGIN_ROOT}/engine/<doc>.md §section)"
notes: ""
```

The `templates` field is **copied from the registry entry** — downstream stages render exactly these templates from `${CLAUDE_PLUGIN_ROOT}/templates/`; designing ad-hoc report layouts is a protocol violation (AGENTS.md Non-Negotiables #3).
...
- **Downstream (REQUIRED NEXT SUB-SKILL)**: `fixed-income-credit-analysis` — after producing the Path Sheet, hand off to this skill for analysis execution; the four-stage chain artifact contract is at `${CLAUDE_PLUGIN_ROOT}/engine/pipeline-contract.md`.

## References

- `references/work-paths.md` — Work path routing view (summary table, with status and one-line trigger characteristics)
- `${CLAUDE_PLUGIN_ROOT}/engine/work-path-registry.md` — Work path registry (single source of truth)

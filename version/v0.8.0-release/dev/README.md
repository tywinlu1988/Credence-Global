...
| **v0.8.0-release** | **2026-07-16** | **集成发布：跨 CLI 通用入口（AGENTS.md + Codex 适配）· 四段链契约（pipeline-contract.md + report/qa 两阶段 skill）· 可执行编排器（src/pipeline.py 接 WP-M4-01/WP-M4-03 两编码引擎）· 维度注册表（dimension-registry.md）。8 条 active 路径端到端走查归档 validation/docs/（append-only 证据）。快照边界扩为 dev/ + AGENTS.md + src/。全量一致性检查与 143 项测试通过。** |

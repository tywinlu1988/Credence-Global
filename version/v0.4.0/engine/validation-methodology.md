...
4. **Track A + Track B双轨独立验证**：同时验证基本面和市场定价两条路径的有效性
...
- **Track A领先于Track B**：基本面信号（Track A）比市场定价（Track B）提前6-12个月发出预警
...
Step 4: 运行框架评估
  ├── Track A: 行业金字塔逐层分析（从L1到L4，不可跳跃）
  │   ├── 每个层级评分 + 一票否决检查
  │   └── 综合评分 + 评级映射
  ├── Track B: 市场定价四级信号（如数据可获取）
  │   ├── 信用利差 / 波动率 / 资金流向 / 评级迁移
...
├── 母公司单体 vs 合并报表的背离程度
├── Track A多层级是否出现2+层同时亮红灯
├── Track B是否出现利差异常扩大（>50bp跳升或持续扩大趋势）
└── 交叉对撞是否处于"分歧"象限（Track A差但Track B好 → 市场忽视风险）
```
...
├── Track B是否开始收敛于Track A（市场终于开始反映基本面）
...
| Track B 市场定价 | 40% | 65% | ↑ | 市场开始反应（从滞后到收敛） |
...
| **无市场数据也不影响** | 一道新能Track B完全为空——但缺失本身就是核心风险信号 |
...
**框架结论（T1）**：4红+1黄（Track A），Track B完全绿色 → "需要持续监控"
**框架结论（T2，违约前4.5个月）**：5红+0黄（Track A升级），Track B开始出现黄色信号
...
**T1→T2信号密度变化**：Track A从75%升至90%，Track B从40%升至65%（市场开始收敛）
...
| **综合评分** | 7.00 | 1.50 |
| **评级** | BB+/BBB- | CCC |
| **Track B** | 🟠 异常（市场悲观） | —（非上市，无市场数据） |
| **区分度** | — | **差距5.5分** |
| **预警时效** | — | 超前实际暴雷8-24个月 |

**核心发现**：框架在同一行业不同信用质量的企业间产生5.5分的差距，具有强判别力。一道新能Track B为空——非上市企业无市场定价本身就是风险信号。
...
| Track A领先Track B | 所有案例 | 基本面信号比市场定价提前6-12个月 |
...
3. **非上市企业的Track B完全不可用**：需要依赖替代信号（招股书、司法记录、招标数据）
